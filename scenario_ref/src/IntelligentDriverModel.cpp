...
void IntelligentDriverModel::updateLane(Car &car, Scenario &scenario) {
    ...
    if (isCarOverJunction(car)) {
        ...
        if (car.advance_data.new_lane_offset != 0) {
            // lane_offset should be validated in this case
            car.moveToLane(*car.getLane()->road.lanes[car.getLane()->lane + car.advance_data.new_lane_offset]);
        }
    }
}
...
void IntelligentDriverModel::moveCarAcrossJunction(Car &car, Scenario &scenario) {
    ...
    int indexOfNextLane = std::min((int)nextRoad->lanes.size() - 1, old_lane->lane + car.advance_data.new_lane_offset);
...
bool IntelligentDriverModel::isCarOverJunction(Car &car) {
    return car.x >= car.getLane()->length;
}
...
double IntelligentDriverModel::calculateWithLead(Car &car, TrafficObject &leading_vehicle)
{
    ...
    double s = std::max(leading_vehicle.x - car.x - leading_vehicle.length, car.min_s);
...
{
    bool isLeadingCarPastTrafficLight = leading_vehicle != nullptr && leading_vehicle->x >= car.getLane()->length - 35. / 2.;
